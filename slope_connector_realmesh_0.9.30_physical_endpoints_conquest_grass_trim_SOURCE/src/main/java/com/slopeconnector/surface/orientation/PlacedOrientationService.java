...
public final class PlacedOrientationService {
    ...
    public static void applyPlacement(ServerWorld world, ServerPlayerEntity player,
        ...
        // Blocks that already expose their own placement orientation must keep vanilla/mod-native
        // placement semantics.  Stairs are the important case; slabs deliberately remain eligible
        // because top/bottom is positional rather than a horizontal facing.
        if (immuneToViewOrientation(original)) {
            remove(world, pos);
            return;
        }
    ...
    private static boolean immuneToViewOrientation(BlockState state) {
        if (state.getBlock() instanceof SlabBlock) return false;
        if (state.getBlock() instanceof StairsBlock) return true;
        for (Property<?> property : state.getProperties()) {
            String name = property.getName();
            if (name.equals("facing") || name.equals("horizontal_facing")
                    || name.equals("axis") || name.equals("horizontal_axis")) return true;
        }
        return false;
    }
