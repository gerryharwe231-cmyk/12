...
public final class ModelBlockEntity extends BlockEntity {
    ...
    /** Physical endpoint widening: side tiles are real ModelBlocks rather than virtual BER copies. */
    private boolean endpointGridMember;
    private boolean endpointGridCompanion;
    private BlockPos endpointGridRoot;
    /** State replaced by this companion; used when a wider endpoint is later shrunk/removed. */
    ...
    public boolean isEndpointGridMember() { return endpointGridMember; }
    public boolean isEndpointGridCompanion() { return endpointGridCompanion; }
    public BlockPos getEndpointGridRoot() { return endpointGridRoot == null ? pos : endpointGridRoot; }
    public BlockState getReplacedState() { return replacedState; }

    public boolean sameEndpointGrid(ModelBlockEntity other) {
        return other != null && endpointGridMember && other.endpointGridMember
                && getEndpointGridRoot().equals(other.getEndpointGridRoot());
    }

    public void setEndpointGrid(BlockPos root, boolean companion, BlockState previous) {
        this.endpointGridMember = true;
        this.endpointGridCompanion = companion;
        this.endpointGridRoot = root == null ? pos : root.toImmutable();
        this.replacedState = previous == null ? Blocks.AIR.getDefaultState() : previous;
        markDirty();
    }

    public void clearEndpointGrid() {
        this.endpointGridMember = false;
        this.endpointGridCompanion = false;
        this.endpointGridRoot = pos;
    ...
    protected void writeNbt(NbtCompound nbt) {
        ...
        nbt.putBoolean("EndpointGridMember", endpointGridMember);
        nbt.putBoolean("EndpointGridCompanion", endpointGridCompanion);
        BlockPos root = getEndpointGridRoot();
        nbt.putInt("EndpointGridRootX", root.getX());
        nbt.putInt("EndpointGridRootY", root.getY());
        nbt.putInt("EndpointGridRootZ", root.getZ());
        nbt.put("EndpointReplacedState", MaterialStateCodec.write(replacedState));
    ...
    public void readNbt(NbtCompound nbt) {
        ...
        endpointGridMember = nbt.getBoolean("EndpointGridMember");
        endpointGridCompanion = nbt.getBoolean("EndpointGridCompanion");
        if (endpointGridMember && nbt.contains("EndpointGridRootX")) {
            endpointGridRoot = new BlockPos(nbt.getInt("EndpointGridRootX"),
                    nbt.getInt("EndpointGridRootY"), nbt.getInt("EndpointGridRootZ"));
        } else endpointGridRoot = pos;
        replacedState = nbt.contains("EndpointReplacedState")
                ? MaterialStateCodec.read(nbt.getCompound("EndpointReplacedState"))
