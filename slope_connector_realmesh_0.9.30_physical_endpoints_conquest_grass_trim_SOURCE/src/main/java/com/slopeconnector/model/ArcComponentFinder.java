...
public final class ArcComponentFinder {
    ...
    private static final int DISCOVERY_RADIUS = 2;
    ...
    private static BlockPos nearestModelBlock(World world, Vec3d point) {
        ...
        BlockPos bestRoot=null,bestAny=null;
        double bestRootDistance=Double.POSITIVE_INFINITY,bestAnyDistance=Double.POSITIVE_INFINITY;
        for(int dx=-ENDPOINT_SEARCH_RADIUS;dx<=ENDPOINT_SEARCH_RADIUS;dx++)
            for(int dy=-ENDPOINT_SEARCH_RADIUS;dy<=ENDPOINT_SEARCH_RADIUS;dy++)
                for(int dz=-ENDPOINT_SEARCH_RADIUS;dz<=ENDPOINT_SEARCH_RADIUS;dz++){
                    BlockPos pos=base.add(dx,dy,dz);
                    if(world.getBlockState(pos).getBlock()!=ModelSystemMod.MODEL_BLOCK)continue;
                    double d=Vec3d.ofCenter(pos).squaredDistanceTo(point);
                    if(d<bestAnyDistance){bestAnyDistance=d;bestAny=pos;}
                    BlockEntity be=world.getBlockEntity(pos);
                    if(be instanceof ModelBlockEntity model && model.isEndpointGridCompanion()) continue;
                    if(d<bestRootDistance){bestRootDistance=d;bestRoot=pos;}
                }
        return bestRoot!=null?bestRoot:bestAny;
    }
