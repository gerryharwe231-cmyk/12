...
public final class ModelRenderWandItem extends Item {
    ...
    private static void skinEndpoint(World world,BlockPos pos,BlockState captured,Direction direction,
        ...
        if(!(world.getBlockEntity(pos) instanceof ModelBlockEntity model) || layout == null)return;
        // Side-width endpoint tiles are physical ModelBlocks, not virtual BER copies.  This keeps
        // render/outline/collision in the same cells and removes the overlap with ArcTrim terrain.
        EndpointCompanionManager.sync(world,pos,layout,lateralTiles,verticalTiles,
                terminalEnd,direction,model.getInnerArcDirection(),captured,true);
