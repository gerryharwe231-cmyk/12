...
public final class ModelStateResolver {
    ...
    /**
     * Source +Q reference used while deforming a captured baked model.  Connected profiles are
     * normalized to a straight state and may use their normalized facing; ordinary models such as
     * stairs/slabs keep the baked orientation exactly as captured, so their source reference stays
     * fixed at world +X rather than being canonicalized by the captured facing a second time.
     */
    public static Direction sourceReferenceDirection(BlockState state) {
        return ConnectionStateHelper.isSupported(state) ? longitudinalDirection(state) : Direction.EAST;
    }

    public static Direction.Axis longitudinalAxis(BlockState state) {
        return sourceReferenceDirection(state).getAxis();
    }
