...
import com.slopeconnector.model.EndpointCompanionManager;
...
public final class ModelBlockRenderer implements BlockEntityRenderer<ModelBlockEntity> {
    ...
    /** Captured endpoint companions often share one BlockState. Decode its BakedQuads only once. */
    private static final Map<BlockState, CachedSource> SOURCE_CACHE = new ConcurrentHashMap<>();
    ...
    public void render(ModelBlockEntity entity, float tickDelta, MatrixStack matrices,
        ...
        try {
            cachedSource = source(client, state);
        } catch (RuntimeException error) {
            state = Blocks.WHITE_CONCRETE.getDefaultState();
            cachedSource = source(client, state);
        }
        if (cachedSource.quads().isEmpty()) return;

        SourceOrientation source = cachedSource.orientation();
        ...
        for (int lateralTile = 0; lateralTile < lateralTiles; lateralTile++) {
            for (int verticalTile = 0; verticalTile < verticalTiles; verticalTile++) {
                for (DecodedQuad quad : cachedSource.quads()) {
                    SourceVertex[] sourceVertices = quad.vertices();
                    if (isBoundaryFace(sourceVertices, Axis.LATERAL, -0.5)
                            && (lateralTile > 0 || EndpointCompanionManager.isSameGridNeighbor(
                            entity.getWorld(), entity, targetLateral, -1)
                            || occludedByFullNeighbor(entity, targetLateral, -1))) continue;
                    if (isBoundaryFace(sourceVertices, Axis.LATERAL, 0.5)
                            && (lateralTile + 1 < lateralTiles || EndpointCompanionManager.isSameGridNeighbor(
                            entity.getWorld(), entity, targetLateral, 1)
                            || occludedByFullNeighbor(entity, targetLateral, 1))) continue;
                    if (isBoundaryFace(sourceVertices, Axis.VERTICAL, -0.5)
                            && (verticalTile > 0 || occludedByFullNeighbor(entity, targetVertical, -1))) continue;
                    if (isBoundaryFace(sourceVertices, Axis.VERTICAL, 0.5)
                            && (verticalTile + 1 < verticalTiles || occludedByFullNeighbor(entity, targetVertical, 1))) continue;
                    // The face looking into the already-generated curved middle is internal.  Keeping
                    // it produces the same coplanar seam/z-fight that vanilla neighboring cubes cull.
                    if (entity.isEndpointGridMember()) {
                        if (!entity.isTerminalEnd() && isBoundaryFace(sourceVertices, Axis.LONGITUDINAL, 1.0)) continue;
                        if (entity.isTerminalEnd() && isBoundaryFace(sourceVertices, Axis.LONGITUDINAL, 0.0)) continue;
                    }
                    ...
                    int tint = 0xFFFFFF;
                    if (quad.tintIndex() >= 0) {
                        int sampled = client.getBlockColors().getColor(state, entity.getWorld(), blockPos, quad.tintIndex());
    ...
    private static CachedSource source(MinecraftClient client, BlockState state) {
        BakedModel model = client.getBlockRenderManager().getModel(state);
        CachedSource cached = SOURCE_CACHE.get(state);
        if (cached != null && cached.model() == model) return cached;
        SourceOrientation orientation = SourceOrientation.of(state);
        List<DecodedQuad> decoded = new ArrayList<>();
        for (BakedQuad quad : collect(model, state)) {
            SourceVertex[] vertices = decode(quad, orientation);
            if (vertices != null) decoded.add(new DecodedQuad(vertices,
                    quad.hasColor() ? quad.getColorIndex() : -1));
        }
        CachedSource built = new CachedSource(model, orientation, List.copyOf(decoded));
        SOURCE_CACHE.put(state, built);
        return built;
    }
    ...
    private static boolean occludedByFullNeighbor(ModelBlockEntity entity, Vec3d positiveAxis, int sign) {
        if (entity.getWorld() == null || positiveAxis == null || positiveAxis.lengthSquared() < EPS) return false;
        Direction direction = dominant(positiveAxis);
        if (sign < 0) direction = direction.getOpposite();
        BlockPos neighborPos = entity.getPos().offset(direction);
        BlockState neighbor = entity.getWorld().getBlockState(neighborPos);
        return !neighbor.isAir() && neighbor.isFullCube(entity.getWorld(), neighborPos);
    }
    ...
    private record SourceOrientation(Vec3d longitudinal, Vec3d lateral) {
        static SourceOrientation of(BlockState state) {
            Direction direction = ModelStateResolver.sourceReferenceDirection(state);
    ...
    private enum Axis { LONGITUDINAL, LATERAL, VERTICAL }
    private record SourceVertex(double q, double lateral, double vertical, float u, float v) {}
    private record DecodedQuad(SourceVertex[] vertices, int tintIndex) {}
    private record CachedSource(BakedModel model, SourceOrientation orientation, List<DecodedQuad> quads) {}
