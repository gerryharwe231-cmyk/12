...
public final class ModelArcRenderer {
    private static final double CURVED_SOURCE_SLICE = 1.0 / 16.0;
    private static final double STRAIGHT_SOURCE_SLICE = 1.0 / 4.0;
    ...
    private static MeshHandle handle(ArcRibbonBlockEntity entity) {
        ...
        BakedModel currentModel = MinecraftClient.getInstance().getBlockRenderManager().getModel(state);
        MeshHandle cached = CACHE.get(entity);
        Integer cachedRevision = cached == null ? null : cached.memberRevisions().get(entity.getPos());
        if (cached != null && cached.state().equals(state) && cached.sourceModel() == currentModel
                && cachedRevision != null && cachedRevision == entity.getRenderRevision()) return cached;
        ArcComponentFinder.Component component = ArcComponentFinder.build(entity);
        if (component == null) return null;
        MeshHandle built = compile(component, currentModel);
        for (ArcRibbonBlockEntity member : component.members()) CACHE.put(member, built);
        return built;
    }

    private static MeshHandle compile(ArcComponentFinder.Component component, BakedModel model) {
        BlockState state = component.leader().getSourceState();
        Map<BlockPos, Integer> revisions = new HashMap<>();
        for (ArcRibbonBlockEntity member : component.members()) revisions.put(member.getPos(), member.getRenderRevision());
        List<BakedQuad> quads = collectQuads(model, state);
        if (quads.isEmpty()) return new MeshHandle(state, model, Map.copyOf(revisions), Map.of());
        ...
        if (total < EPS) return new MeshHandle(state, model, Map.copyOf(revisions), Map.of());
        ...
        return new MeshHandle(state, model, Map.copyOf(revisions), Map.copyOf(frozen));
    }
    ...
    private static void subdivideAndWarp(List<SourceVertex> source, BakedQuad quad,
        ...
        double sourceSlice = curve.isNearStraight(moduleStart, moduleLength)
                ? STRAIGHT_SOURCE_SLICE : CURVED_SOURCE_SLICE;
    ...
    private static final class CurveSampler {
        ...
        boolean isNearStraight(double start, double length) {
            Frame a = sample(Math.max(0.0, start));
            Frame b = sample(Math.min(total, start + length));
            Frame m = sample(Math.min(total, start + length * 0.5));
            return a.tangent().dotProduct(b.tangent()) > 0.9995
                    && a.tangent().dotProduct(m.tangent()) > 0.9995
                    && m.tangent().dotProduct(b.tangent()) > 0.9995;
        }
    ...
    private record MeshHandle(BlockState state, BakedModel sourceModel,
